    ...
    SubShader
    {
		...
		Pass
        {
            ...
            FragmentOutput frag(v2f i)
            {
				...
				// Tex Sample
                half4 mainTex = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, UV * _BaseMap_ST.xy);
                half4 pbrMask = SAMPLE_TEXTURE2D(_PBRMask, sampler_PBRMask, UV);
				half3 bumpTS = UnpackNormalScale(SAMPLE_TEXTURE2D(_NormalMap, sampler_NormalMap, UV * _BaseMap_ST.xy / 2), _NormalScale);
				...
				// #endif /* _DIRECT_BLINNPHONG */
                    ...
                    float2 specRampUV = float2(blinnPhongSpec, 0.375);
